...
@section('content')
<form action="{{route('farmland.update',$farm->id)}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
            ...
            <div class="form-group row mt-4">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control boxed" name="size" value="{{ $farm->size }}">
                    </div>
            </div>
             ...
             <div class="form-group row">
                ...
                <div class="col-sm-8">
                    <select name="location_id" id="" class=" chosen-select form-control boxed ">
                        @foreach ($local as $l)
                        <option value="{{ $l->id }}" {{ $l->id==$farm->location_id?'selected':'' }}>{{ $l->name }}</option>
                    @endforeach
                </select>
                </div>
             </div>
             <div class="form-group row">
                ...
                <div class="col-sm-8">
                    <textarea name="potaintial" id="" cols="30" rows="2" class="form-control boxed">{{ $farm->potaintial }}</textarea>
                </div>
             </div>
             <div class="form-group row">
                <label for="Description" class="col-sm-4">Description</label>
                <div class="col-sm-8">
                    <textarea name="description" id="" cols="30" rows="2" class="form-control boxed">{{ $farm->description }}</textarea>
                </div>
             </div>
             <div class="form-group row">
                <label for="for" class="col-sm-4">Price <span class="text-primary">($)</span></label>
                <div class="col-sm-8">
                  <input type="text" name="price" id="" class="form-control boxed" value="{{ $farm->price }}">
                </div>
             </div>
             <div class="form-group row">
                ...
                <div class="col-sm-8">
                   <select name="for_id" id="" class="form-control boxed">
                            <option value=""></option>
                            @foreach ($for as $f)
                           <option value="{{$f->id}}" {{$f->id==$farm->for_id?'selected':''}}>{{$f->name}}</option>
                            @endforeach
                   </select>
                </div>
             </div>
            ...
            <div class="mt-4">
                ...
                <div class="mt-3">
                    <img src="{{ asset($farm->photo) }}" alt="" width="200" id="img" class="">
                </div>
            </div>
